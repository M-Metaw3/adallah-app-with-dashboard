...
class User {
    static register = async (req, res) => {
        ...
        const { confirmPassword, userType, name, arName, password, nationality, phone,countryCallingCode, image } = req.body
        if (password != confirmPassword) return res.status(400).json({
            message: "password doesnt match"
        })
        // const hashPassword = await bycrpt.hash(password,12)
        try {
            const myCountryCodesObject = countryCodes.customList('countryCallingCode', '{officialLanguageCode}-{countryCode}')
            const user = new userModel({
                phone: myCountryCodesObject[countryCallingCode.substring(1)]+":"+countryCallingCode+phone,
                userType: userType,
                name: name,
                arName,
            ...
            const token = await creatToken({ id: result._id, userType: result.userType },true)
    ...
    static signin = async (req, res) => {
        ...
        const myCountryCodesObject = countryCodes.customList('countryCallingCode', '{officialLanguageCode}-{countryCode}')
        const isexsitedphone = await userModel.findOne({ phone:myCountryCodesObject[countrycode.substring(1)]+":"+countrycode+phone })
        if (!isexsitedphone) return res.status(400).json({ message: "phone not exsited" })
        ...
        try {
            ...
            //   const  token =await jwt.sign({id:result._id,userType:result.userType},"test",{expiresIn:"5h"})
            const token = await creatToken({ id: isexsitedphone._id, userType: isexsitedphone.userType }, true)
            res.header("authorization", token)
            res.status(201).json({
                message: "loged in"
                ,
